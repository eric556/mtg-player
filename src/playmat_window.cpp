...
// On resize the playmat simply extends to fill the new window —
// no letterboxing, no black bars. Cards keep their absolute positions
// and more green felt becomes visible as the window grows.
...
// ── Pile stack helper ──────────────────────────────────────────────────────
// Draws a back-face card stack at `center` with a count badge and label.
...
PlaymatWindow::PlaymatWindow(GameState& gs)
    : state_(gs)
{
    window.create(sf::VideoMode({static_cast<unsigned>(PLAYMAT_W),
                                 static_cast<unsigned>(PLAYMAT_H)}),
    ...
    // Pile viewer overlays — centred on the playmat
...
{
    // Pile viewers consume all clicks while open.
    ...
    if (btn == sf::Mouse::Button::Left) {
        ...
        // GY / exile pile clicks.
        if (gy_rect_.contains(p) && !state_.graveyard.empty()) {
            gy_viewer_.show("Graveyard", state_.graveyard);
            return;
        }
        if (exile_rect_.contains(p) && !state_.exile.empty()) {
            exile_viewer_.show("Exile", state_.exile);
            return;
        }

        // Card drag / double-click tap.
        int idx = cardAt(p);
        if (idx >= 0) {
...
void PlaymatWindow::render()
{
    ...
    // ── GY and exile piles (top-right) ────────────────────────────────────
    drawPileStack(window, fp, {PM_GY_CX, PM_GY_CY},
                  static_cast<int>(state_.graveyard.size()),
                  "GY", sf::Color(110, 45, 45, 210));

    drawPileStack(window, fp, {PM_EXILE_CX, PM_EXILE_CY},
                  static_cast<int>(state_.exile.size()),
                  "EXILE", sf::Color(110, 80, 25, 210));

    if (fp) {
        // Small "click to view" hints under each pile when non-empty.
