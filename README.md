...
- `--vcam <output>` — stream via ffmpeg; `<output>` is any ffmpeg output path or URL
- `--vcam-native` — Windows 11 virtual webcam via `IMFVirtualCamera` (no ffmpeg, no OBS needed; registers the COM source DLL on first run)
...
**Virtual camera — Windows 11 (no OBS, no ffmpeg):**

```
mtg-sim --deck my_deck.txt --vcam-native
```

On first run the app registers a small COM DLL (`mtg-sim-vcam.dll`, built alongside the exe) under your user account — no admin prompt needed. The playmat then appears as **"MTG Sim"** in SpellTable's camera picker and in Windows camera settings.

**Virtual camera — Linux / file / RTMP (ffmpeg required):**

```
# Linux virtual webcam (requires v4l2loopback: modprobe v4l2loopback)
